...
function isValidCronField(field: string, min: number, max: number): boolean {
  ...
  if (field.startsWith("*/")) return isValidNumber(field.slice(2)) && Number(field.slice(2)) > 0;
  if (field.includes("-")) {
    const [start, end] = field.split("-");
    return Boolean(start && end && isValidNumber(start) && isValidNumber(end) && Number(start) <= Number(end));
  }
  if (field.includes(",")) return field.split(",").every(isValidNumber);
...
export default function CronSettingsPage() {
  ...
  const applyDraft = (patch: CronConfigPatch): Promise<void> => {
    setSaving(true);
    showSaving();
    setError(null);
    return api
      ...
      .then((updated) => {
        ...
        setScanDirty(false);
        setTickDirty(false);
        setInstallMode(((updated as CronConfigResponse).install_mode || "auto") as "auto" | "manual");
        setLastApplied({
  ...
  const scanChanged = config ? draftScanCron.trim() !== config.scan_cron : false;
  const tickChanged = config ? draftTickCron.trim() !== config.tick_cron : false;
  const scanCronValid = isValidCron(draftScanCron);
  const tickCronValid = isValidCron(draftTickCron);
  const hasCronChanges = scanChanged || tickChanged;
  const canApplyCronChanges = hasCronChanges && scanCronValid && tickCronValid;

  const handleApply = () => {
    if (!canApplyCronChanges) return;
  ...
  const handlePreset = (scanVal: string, tickVal: string) => {
    ...
    setScanDirty(scanVal.trim() !== config?.scan_cron);
    setTickDirty(tickVal.trim() !== config?.tick_cron);
  };

  const handleScanChange = (val: string) => {
    setDraftScanCron(val);
    setScanDirty(val.trim() !== config?.scan_cron);
  };

  const handleTickChange = (val: string) => {
    setDraftTickCron(val);
    setTickDirty(val.trim() !== config?.tick_cron);
  };

  const handleToggle = (checked: boolean) => {
    setDraftEnabled(checked);
    // Only apply if the value actually changed from the server config
    if (checked !== config?.enabled) {
      applyDraft({ enabled: checked }).catch(() => {
        // Error already handled in applyDraft
      });
    }
  };
  ...
  const handleInstallModeChange = (mode: "auto" | "manual") => {
    setInstallMode(mode);
    // Only apply if the value actually changed from the server config
    if (mode !== config?.install_mode) {
      applyDraft({ install_mode: mode }).catch(() => {
        // Error already handled in applyDraft
      });
    }
  };
  ...
  useUnsavedWarning({ isDirty: hasCronChanges });
      ...
      <div className="w-full max-w-3xl space-y-6">
          ...
          <div className="mt-4 space-y-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder="*/15 * * * *"
                className={`mt-1 w-full rounded-lg border bg-[var(--panel-2)] px-3 py-2 text-sm font-mono text-[var(--text)] outline-none focus:border-cyan-400/60 ${scanDirty && !scanCronValid ? "border-red-400/50" : "border-[var(--border)]"} disabled:opacity-50`}
              />
            ...
            <div>
              ...
              <input
                ...
                disabled={!draftEnabled || saving}
                placeholder="0 * * * *"
                className={`mt-1 w-full rounded-lg border bg-[var(--panel-2)] px-3 py-2 text-sm font-mono text-[var(--text)] outline-none focus:border-cyan-400/60 ${tickDirty && !tickCronValid ? "border-red-400/50" : "border-[var(--border)]"} disabled:opacity-50`}
              />
              <div className="mt-1 text-xs text-[var(--text-dim)]">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (false !== config?.enabled) {
                      applyDraft({ enabled: false }).catch(() => {
                        // Error already handled in applyDraft
                      });
                    }
                  }}
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            {hasCronChanges && saveFeedback === "idle" && (
          ...
          <button
            type="button"
            onClick={handleApply}
            disabled={saving || !canApplyCronChanges}
